...
function isOverIntRange(val, type) {
  switch (type) {
    case 'int32':
...
function getOptionalParams(req, option_expressions) {
  ...
  for (var i = 0, li = option_expressions.length; i < li; i++) {
    ...
    var func = typeMap[keyInfo.type];
    if (func) {
      if (val !== undefined && val !== '') {
        options[key] = func(val);
      } else if (keyInfo.defaultVal) {
        options[key] = func(keyInfo.defaultVal);
      }
      ...
      if (val) {
        ...
        if (['int', 'int32', 'uint32'].includes(keyInfo.type) && !_.isSafeInteger(options[key])) {
          err = new Error('The parameter value is not a integer : ' + key);
          err.code = 400;
          break;
        }

        if (['int32', 'uint32'].includes(keyInfo.type) && isOverIntRange(options[key], keyInfo.type)) {
          err = new Error('The parameter value is over the ' + keyInfo.type + ' range');
          err.code = 400;
          break;
        }
      }
    } else {
      err = new Error('The ' + keyInfo.type + ' is an unsupported type.');
      err.code = 400;
      break;
    }
  }
...
function requiredParameter(req, required_expressions) {
  ...
  for (var i = 0, li = required_expressions.length; i < li; i++) {
    ...
    if (keyInfo.pathKey) getFunc = getPath;
    else if (extraOption[key]) getFunc = getExtra;
    else getFunc = getValue;
    ...
    if (keyInfo.type && keyInfo.type !== 'string') {
      var func = typeMap[keyInfo.type];
      if (!func) {
        err = new Error('The ' + keyInfo.type + ' is an unsupported type.');
        err.code = 400;
      ...
      if (['int', 'int32', 'uint32'].includes(keyInfo.type) && !_.isSafeInteger(val)) {
        err = new Error('The parameter value is not a integer : ' + key);
        err.code = 400;
        break;
      }

      if (['int32', 'uint32'].includes(keyInfo.type) && isOverIntRange(val, keyInfo.type)) {
        err = new Error('The parameter value is over the ' + keyInfo.type + ' range');
        err.code = 400;
        break;
      }
